...
export async function POST(request: Request) {
  try {
    ...
    const input = z.object({ subjectType: z.enum(["company", "product"]), subjectId: z.uuid(), method: z.enum(["dns", "email", "github", "oauth", "signed_challenge"]), target: z.string().max(300).optional(), provider: z.string().max(80).optional(), publicKey: z.string().max(4_000).optional() }).parse(await request.json());
    const result = await createVendorClaim({ ...input, organisationId: organisation.id, userId: session.user.id });
    if (input.method === "email" && process.env.NODE_ENV !== "production") console.info(`[TrustForge development email challenge] ${input.target}: ${result.challenge}`);
