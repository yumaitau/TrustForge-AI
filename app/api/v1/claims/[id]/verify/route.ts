...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const proof = z.object({ challenge: z.string().min(20).max(200).optional(), code: z.string().min(6).max(200).optional(), signature: z.string().max(2_000).optional(), providerAttestation: z.string().max(500).optional() }).parse(await request.json());
